...
def test_new_tab(session, test_url):
    ...
    # session.execute_script("Object.defineProperty(navigator, 'webdriver', {get: () => undefined})")

    # Navigate to TikTok
    session.get(test_url)
    WebDriverWait(session, 10).until(EC.url_contains("https://www.tiktok.com"))

    # Verify the TikTok page is loaded
    WebDriverWait(session, 10).until(EC.title_contains("videos on TikTok"))
    ...
    item_element = WebDriverWait(session, 10).until(
        EC.presence_of_element_located(
            (By.XPATH, "//div[1]/div[2]/div[2]/div[1]/div[2]/div/div[1]/a[2]/p[2]/strong")))
