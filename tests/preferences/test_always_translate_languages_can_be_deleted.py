...
def test_always_translate_languages_can_be_deleted(
    driver: Firefox, about_prefs: AboutPrefs, german_page: str
):
    """
    C3399165 - Languages can be deleted from the 'Always translate these languages' section.
    """
    ...
    about_prefs.add_always_translate_language(DROPDOWN_LANGUAGE)
    ...
    # The page language is listed back on the Translations sub-pane.
    about_prefs.open()
    about_prefs.open_more_translation_settings()
    about_prefs.element_visible("always-translate-item", labels=[PAGE_LANGUAGE])

    # Deleting it removes only that row.
    about_prefs.remove_always_translate_language(PAGE_LANGUAGE)
    about_prefs.element_does_not_exist("always-translate-item", labels=[PAGE_LANGUAGE])
    about_prefs.element_visible("always-translate-item", labels=[DROPDOWN_LANGUAGE])
