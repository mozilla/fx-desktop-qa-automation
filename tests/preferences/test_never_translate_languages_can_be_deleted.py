...
def test_never_translate_languages_can_be_deleted(
    driver: Firefox, about_prefs: AboutPrefs, german_page: str
):
    """
    C3399168 - Languages can be deleted from the 'Never translate these languages' section.
    """
    ...
    about_prefs.open()
    about_prefs.open_more_translation_settings()
    about_prefs.element_visible("never-translate-item", labels=[PAGE_LANGUAGE])

    # Deleting it removes only that row.
    about_prefs.remove_never_translate_language(PAGE_LANGUAGE)
    about_prefs.element_does_not_exist("never-translate-item", labels=[PAGE_LANGUAGE])
    about_prefs.element_visible("never-translate-item", labels=[DROPDOWN_LANGUAGE])
