...
def test_verify_telemetry_downloads_panel_open(driver: Firefox):
    ...
    # Reload telemetry page and verify that telemetry "downloads.panel_shown: 11"
    nav.refresh_page()
    assert telemetry.is_telemetry_scalars_entry_present(
        DOWNLOADS_PANEL_TELEMETRY_DATA_RELOAD
    )

    # Open Telemetry keyed scalars
    telemetry.click_on("keyed-scalars-tab")

    # Verify telemetry "downloads-button: 10"
    assert telemetry.is_telemetry_keyed_scalars_entry_present(
        DOWNLOADS_PANEL_TELEMETRY_KEYED_SCALARS
    )
