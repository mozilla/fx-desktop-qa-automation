...
def _css_value(element: WebElement, prop: str) -> int:
    """
    Read a CSS property and normalize it to a float by stripping non-numeric chars.
    """
    val = element.value_of_css_property(prop)
    val = "".join(char for char in val if char.isnumeric() or char == ".")
    return float(val)
...
def test_type_control_panel_size(driver: Firefox, control: SizeControl) -> None:
    ...
    body = web_page.get_element("page-body")
    size_before = _css_value(body, "--font-size")
...
def test_type_control_panel_content_width(
    ...
    body = web_page.get_element("page-body")
    width_before = _css_value(body, "--content-width")
    ...
    if direction == "decrease":
        reader_view.wait.until(
            lambda _: _css_value(body, "--content-width") < width_before
        )
    else:
        reader_view.wait.until(
            lambda _: _css_value(body, "--content-width") > width_before
...
def test_type_control_panel_line_spacing(
    ...
    body = web_page.get_element("page-body")
    block_before = _css_value(body, "block-size")
    slider = reader_view.get_element("slider")
...
def test_type_control_panel_character_spacing(driver: Firefox) -> None:
    ...
    container = web_page.get_element("container-div")
    letter_before = _css_value(container, "--letter-spacing")
    slider = reader_view.get_element("slider")

    reader_view.change_slider_value(slider, increase=True)

    reader_view.wait.until(
        lambda _: _css_value(container, "--letter-spacing") > letter_before
    )
...
def test_type_control_panel_word_spacing(driver: Firefox) -> None:
    ...
    container = web_page.get_element("container-div")
    word_before = _css_value(container, "--word-spacing")
    slider = reader_view.get_element("slider")

    reader_view.change_slider_value(slider, increase=True)

    reader_view.wait.until(
        lambda _: _css_value(container, "--word-spacing") > word_before
    )
