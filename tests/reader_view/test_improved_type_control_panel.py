...
# Helpers
def _open_reader_type_panel(web_page: GenericPage,
                            reader_view: ReaderView) -> None:
    """
...
def _css_int(util: Utilities, element, prop: str) -> int:
    ...
    """
    return int(util.remove_all_non_numbers(
        element.value_of_css_property(prop)))


@pytest.mark.parametrize("font", FONTS)
def test_type_control_panel_font(
        driver: Firefox, font: Literal["sans-serif", "serif",
                                       "monospace"]) -> None:
    """
    ...
    font_dropdown = Dropdown(page=reader_view,
                             require_shadow=False,
                             root=font_dropdown_root)
    ...
    reader_view.wait.until(
        lambda _: font in body.value_of_css_property("font-family"))


@pytest.mark.parametrize("control", SIZE_CONTROLS)
def test_type_control_panel_size(driver: Firefox,
                                 control: SizeControl) -> None:
    """
...
def test_type_control_panel_text_alignment(
    ...
    reader_view.wait.until(lambda _: container.value_of_css_property(
        "--text-alignment") == intended_alignment)


@pytest.mark.parametrize("direction", SLIDER_DIRS)
def test_type_control_panel_content_width(driver: Firefox,
                                          direction: SliderDirection) -> None:
    """
...
@pytest.mark.parametrize("direction", SLIDER_DIRS)
def test_type_control_panel_line_spacing(driver: Firefox,
                                         direction: SliderDirection) -> None:
    """
...
def test_type_control_panel_character_spacing(driver: Firefox) -> None:
    ...
    reader_view.change_slider_element_shadow_parent(
    ...
    reader_view.wait.until(lambda _: _css_int(
        util, container, "--letter-spacing") > letter_before)
