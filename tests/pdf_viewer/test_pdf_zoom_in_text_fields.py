...
def test_pdf_zoom_in_text_fields(pdf_viewer: GenericPdf):
    """
    ...
    before_zoom_in_scale_factor = float(body.value_of_css_property("--scale-factor"))
    ...
    before_zoom_out_scale_factor = float(body.value_of_css_property("--scale-factor"))

    pdf_viewer.zoom_out_toolbar()
    pdf_viewer.zoom_out_toolbar()
    pdf_viewer.wait.until(
        lambda _: float(body.value_of_css_property("--scale-factor"))
        < before_zoom_out_scale_factor
    )
    pdf_viewer.element_attribute_is(TEXT_FIELD, "value", TEST_TEXT)
